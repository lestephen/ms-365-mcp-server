...
function buildGeneralMcpInstructions(opts: McpInstructionsContext): string {
  const parts = [
    ...
    'Files / binary content: prefer get-download-url for large content. It returns native pre-authenticated URLs for drive/SharePoint files and, when the broker is configured, short-lived brokered URLs for mail and calendar attachments. Use download-bytes for small authenticated byte reads such as attachments, profile photos, Teams hosted content, and meeting recordings. In stdio mode, download-bytes-to-file writes authenticated bytes straight to a local absolute path and remains the out-of-band option for meeting recordings. These tools take relative Microsoft Graph paths, not absolute URLs. For uploads, prefer create-upload-session and send the bytes to the returned uploadUrl yourself, streaming them from disk; this server does not perform that PUT. upload-file-content instead carries the entire file as a base64 tool argument, and a truncated argument decodes to a truncated file with no error, so do not build that string regardless of how small the file looks.',
  ];
