...
describe('Read-Only Mode', () => {
  ...
  it('should respect --read-only flag from CLI', () => {
    ...
    // 1 GET endpoint + parse-teams-url + download-bytes + get-download-url + copilot-retrieve utility tools
  ...
  it('should register all endpoints when not in read-only mode', () => {
    ...
    // 4 mocked endpoints (get-schedule skipped: workScopes only, no orgMode) + parse-teams-url + download-bytes + get-download-url + copilot-retrieve
  ...
  it('should allow POST endpoints with readOnly: true in endpoints.json in read-only mode', () => {
    ...
    // 2 graph tools (list-mail-messages + get-schedule) + parse-teams-url + download-bytes + get-download-url + copilot-retrieve
